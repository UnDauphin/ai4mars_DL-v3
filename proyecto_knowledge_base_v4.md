...
>
> **Versión**: 4 — Refleja todas las decisiones tomadas hasta la sesión de implementación de modelos.
> Sustituye completamente la v3. Todo lo marcado como ~~tachado~~ o `[DEPRECADO]` en este documento ya no aplica.
...
**Por qué CUDA 12.4**: El driver 596.36 soporta hasta CUDA 13.2, pero PyTorch 2.5.1 con cu124 es la combinación más estable disponible para RTX 4050.
...
> ⚠️ **Corregido respecto a v2**: El TDP es 35W (no 50W). Confirmado por `nvidia-smi`.
...
| Implementación modelos | 30% | ⚠️ En progreso — `05b_model_segformer.ipynb` generado; 05a, 05c, 05d, 05e pendientes |
...
*Documento actualizado — v4. Reemplaza completamente la v3.*
*Últimos cambios: `mars_utils.py` reescrito (v4) con normalización propia, rutas robustas local/Colab, `make_fast_subset`, `plot_best_seed_curves`, `print_summary_table`; `05b_model_segformer.ipynb` generado; arquitecturas de todos los modelos confirmadas y documentadas; orden de implementación definido; marco teórico completado y corregido.*
*Próximos pasos: notebooks 05a, 05c, 05d, 05e en paralelo entre compañeros.*
